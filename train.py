...
from losses import dice_loss,rbox_loss,quad_loss
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.epochs_since_last_save >= self.period:
            ...
            if self.save_weights_only:
                self.model_for_saving.save_weights(self.path.format(epoch=epoch + FLAGS.last_epoch_train+ 1, **logs), overwrite=True)
            else:
                self.model_for_saving.save(self.path.format(epoch=epoch + FLAGS.last_epoch_train+1, **logs), overwrite=True)
...
class ValidationEvaluator(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if (epoch + 1) % self.period == 0:
            ...
            print('\nEpoch %d: val_loss: %.4f, val_score_map_loss: %.4f, val_geo_map_loss: %.4f' % (epoch + FLAGS.last_epoch_train+1, val_loss, val_score_map_loss, val_geo_map_loss))
            with self.val_writer.as_default():
                tf.summary.scalar('loss',val_loss,step = epoch+FLAGS.last_epoch_train+1)
                tf.summary.scalar('pred_score_map_loss',val_score_map_loss,step = epoch+FLAGS.last_epoch_train+1)
                tf.summary.scalar('pred_geo_map_loss',val_geo_map_loss,step = epoch+FLAGS.last_epoch_train+1)
...
def main(argv = None):
    ...
    callbacks = [lr_scheduler, ckpt, tb, validation_evaluator]

    opt = Adam(learning_rate=FLAGS.init_learningrate)
    ...
    history = Model.fit_generator(train_data_generator,epochs=FLAGS.max_epochs,steps_per_epoch = train_samples_count/FLAGS.batch_size, workers=FLAGS.nb_workers, use_multiprocessing=True, max_queue_size=10, callbacks=callbacks, verbose=1)