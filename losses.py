...
def quad_loss(overly_small_text_region_training_mask, text_region_boundary_training_mask, loss_weight, small_text_weight):
    def loss(y_true,y_pred):
       pass
    return loss
...
def rbox_loss(overly_small_text_region_training_mask, text_region_boundary_training_mask, small_text_weight, target_score_map):
    def loss(y_true, y_pred):
        ...
        w_union = tf.minimum(d2_gt, d2_pred) + tf.maximum(d4_gt, d4_pred)
        h_union = tf.minimum(d1_gt, d1_pred) + tf.maximum(d3_gt, d3_pred)
